...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		apiRouter.POST("/setup", middleware.SetupAccessGuard(), anonymousRequestBodyLimit, controller.PostSetup)
		...
		tokenRoute.Use(middleware.TokenOperationAudit())
		{
			...
			tokenRoute.POST("/:id/key", middleware.UserCriticalRateLimit("token-key-read"), middleware.DisableCache(), controller.GetTokenKey)
			tokenRoute.POST("/", controller.AddToken)
			tokenRoute.PUT("/", controller.UpdateToken)
			tokenRoute.DELETE("/:id", controller.DeleteToken)
			tokenRoute.POST("/batch", controller.DeleteTokenBatch)
			tokenRoute.POST("/batch/keys", middleware.UserCriticalRateLimit("token-key-read"), middleware.DisableCache(), controller.GetTokenKeysBatch)
		}
